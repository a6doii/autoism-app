...
const PatternGame = ({ onFinish, onBack }) => {
  ...
  const next = () => {
    if (ri + 1 >= TOTAL) {
      setState('gameover');
      const pctP = score / TOTAL;
      if (pctP >= 0.8) { sounds.win(); confetti({ particleCount: 200, spread: 120, origin: { y: 0.4 }, zIndex: 1000 }); }
      else if (pctP >= 0.5) { sounds.pass(); confetti({ particleCount: 200, spread: 120, origin: { y: 0.4 }, zIndex: 1000 }); }
      else { sounds.lose(); }
